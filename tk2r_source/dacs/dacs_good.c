...
FTRep* createFT(uint *list,uint listLength){
	...
	kvalues = optimizationk(list,listLength,(int *)&nkvalues);
	...
	for (i=0;i<listLength;i++){
		value = list[i];
		for(j=0;j<(int)rep->tamtablebase;j++)
	...
	while((j<(int)rep->tamtablebase)&&(levelSizeAux[j]!=0)){
		 j++;
		}
	rep->nLevels = j;
...
FTRep* loadFT(char * filename){
	...
	result = fread(&(rep->listLength),sizeof(uint),1,flist);  if (result != 1) {fputs ("Reading error", stderr); exit(EXIT_FAILURE);}
	result = fread(&(rep->nLevels),sizeof(byte),1,flist);     if (result != 1) {fputs ("Reading error", stderr); exit(EXIT_FAILURE);}
	result = fread(&(rep->tamCode),sizeof(uint),1,flist);     if (result != 1) {fputs ("Reading error", stderr); exit(EXIT_FAILURE);}
	
	result = fread(&(rep->tamtablebase),sizeof(uint),1,flist); if (result != 1) {fputs ("Reading error", stderr); exit(EXIT_FAILURE);}
	rep->tablebase = (uint *) malloc(sizeof(uint)*rep->tamtablebase);
	result = fread(rep->tablebase,sizeof(uint),rep->tamtablebase,flist); if (result != rep->tamtablebase) {fputs ("Reading error", stderr); exit(EXIT_FAILURE);}
	
	rep->base_bits = (ushort *) malloc(sizeof(ushort)*rep->nLevels);
	result = fread(rep->base_bits,sizeof(ushort),rep->nLevels,flist); if (result != rep->nLevels) {fputs ("Reading error", stderr); exit(EXIT_FAILURE);}
	
	
	rep->base = (uint *) malloc(sizeof(uint)*rep->nLevels);
	result = fread(rep->base,sizeof(uint),rep->nLevels,flist); if (result != rep->nLevels) {fputs ("Reading error", stderr); exit(EXIT_FAILURE);}
	
	
	rep->levelsIndex = (uint *) malloc(sizeof(uint)*(rep->nLevels+1));
	result = fread(rep->levelsIndex,sizeof(uint),rep->nLevels+1,flist); if (result != (size_t)rep->nLevels+1) {fputs ("Reading error", stderr); exit(EXIT_FAILURE);}
	
	rep->iniLevel = (uint *) malloc(sizeof(uint)*rep->nLevels);
	result = fread(rep->iniLevel,sizeof(uint),rep->nLevels,flist); if (result != rep->nLevels) {fputs ("Reading error", stderr); exit(EXIT_FAILURE);}

	rep->rankLevels = (uint *) malloc(sizeof(uint)*(rep->nLevels));
	result = fread(rep->rankLevels,sizeof(uint),rep->nLevels,flist); if (result != rep->nLevels) {fputs ("Reading error", stderr); exit(EXIT_FAILURE);}
	
	rep->levels = (uint *) malloc(sizeof(uint)*(rep->tamCode/W+1));	
	result = fread(rep->levels,sizeof(uint),(rep->tamCode/W+1),flist); if (result != (rep->tamCode/W+1)) {fputs ("Reading error", stderr); exit(EXIT_FAILURE);}
