...
ushort * optimizationk(uint * list,int listLength, int * nkvalues){
	...
	//uint maxInt=0; // modified by Kevin
	int maxInt=0;
	//int maxInt = -2147483648; //-32768;
	
	for(i=0;i<listLength;i++)
		if(maxInt<(int)list[i]) 
			maxInt = list[i];
...
FTRep* createFT(uint *list, uint listLength)
{
	...
	//int hist[64*1024] = {0};
	//int histlen = makehist(list, hist, listLength);
	//double entr = entropy(hist, histlen, listLength);
	//printf("Entropy: %f\n", entr);
//printf("createFT.A\n");		
	...
	kvalues = optimizationk(list,listLength, (int *)&nkvalues); // modified by K.C. 30/4/2018
