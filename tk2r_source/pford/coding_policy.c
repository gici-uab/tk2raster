...
int compress_pfordelta(unsigned int *input, unsigned int *output, int num_input_elements, int block_size_) {
  ...
  while (num_whole_blocks-- > 0) {
	...
	////printf("input+unencoded_offset %08x\tencoded_offset: %d\n", input[unencoded_offset], encoded_offset);
...
int decompress_pfordelta(unsigned int* input, unsigned int* output, int num_input_elements, int _block_size, int pos) {
  ...
  while (num_whole_blocks-- > 0) {
    encoded_offset += pfor_decompress(input + encoded_offset, output + unencoded_offset, _block_size, pos);
